...
void LinRotAccelerator::overrideSpeed(float factor)
{
    ...
    noInterrupts();
    v_tgt = v_tgt_orig * factor; //velocity target = velocity target org * facotr
    dv = v_tgt > v_cur ? dv_cur : -dv_cur; //velocity target > velocity current -> dv=dv_cur otherwise dv=-dv_cur
    interrupts();
}
