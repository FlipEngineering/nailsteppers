...
MiniPID::MiniPID(double p, double i, double d){
	...
	//https://github.com/blandown/MiniPID/commit/b257cab16ce3702e68fbf7808d0b18dc8624ab4f?branch=b257cab16ce3702e68fbf7808d0b18dc8624ab4f&diff=split
	//---new commit 
	setP(p);
    setI(i);
    setD(d);
...
double MiniPID::getOutput(double actual, double setpoint){
	...
	//https://github.com/tekdemo/MiniPID/issues/8
	...
	//--- included from Fork, will now join with above issue commit of error - last error
	...
	if(useDt && (dt > 0)){
		...
		//Doutput= -D*(actual-lastActual); // DERIVATION ON MEASURMENT - deriv error = neg deriv of input, exccept when setpoint is changing
		Doutput = D*(error - lastError) / dt; // DERIVATION KICK <----------- seems right ish, not sure might make problems
		lastActual = actual;
		lastSetpoint = setpoint;
	} else{
		//Doutput= -D*(actual-lastActual);
